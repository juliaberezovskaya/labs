...
std::ostream& operator<<(std::ostream& os, const CalculateFraction& fraction)
{
	os << fraction.m_numerator << '/' << fraction.m_denominator << " calculation: " << fraction.c_calculation;
	return os;
}

bool operator==(const CalculateFraction& left, const CalculateFraction& right)
{
	if (left.m_numerator == right.m_denominator &&
		left.m_denominator == right.m_denominator) return true;
	return false;
}

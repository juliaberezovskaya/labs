...
std::ostream& operator<<(std::ostream& os, const MixedFraction& fraction)
{
	os << fraction.m_whole_part << ' ' << fraction.m_numerator << '/' << fraction.m_denominator;
	return os;
}

bool operator==(const MixedFraction& left, const MixedFraction& right)
{
	if (left.m_numerator == right.m_denominator && 
		left.m_denominator == right.m_denominator && 
		left.m_whole_part == right.m_whole_part) return true;
	return false;
}
